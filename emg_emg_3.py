# train_all_combinations_three_inputs.py
...
# --- Hyperparameters Adjusted for New Task ---
SEQUENCE_LEN = 3
LEARNING_RATE = 0.001
BATCH_SIZE = 64
EPOCHS = 100
LSTM_HIDDEN = 128
DROPOUT_RATE = 0.5
NUM_INPUT_CHANNELS = 3 # CHANGED: Now using 3 input channels
...
if __name__ == "__main__":
    ...
    print(f"Found {len(all_combinations)} unique combinations to test (3 inputs -> 1 target).")
    ...
    for i, combo in enumerate(all_combinations):
        ...
        # CHANGED: Updated formatting for 3 channels
        results.append({
            'input_channels': f"{combo['inputs'][0]}, {combo['inputs'][1]}, {combo['inputs'][2]}",
            'target_channel': combo['target'],
            'r_squared': r2,
            'mse': mse,
            'correlation': corr
        })
