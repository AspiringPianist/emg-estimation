...
BEST_INPUT_CHANNELS = ['emg1', 'emg7', 'emg2']  # <-- Top 3 inputs
BEST_TARGET_CHANNEL = 'emg8'                    # <-- Corresponding target (used internally only)
# ----------------------------------------------------------------

# --- STEP 2: DEFINE WHERE TO SAVE THE PLOTS ---
MAIN_PLOT_DIRECTORY = 'C:/Users/rajes/Desktop/IIITB/emg-emg/PLOT_RESULTS_THREE_INPUTS/'
# ----------------------------------------------------------------

# --- Hyperparameters (consistent with the main experiment) ---
SEQUENCE_LEN = 3
LEARNING_RATE = 0.001
BATCH_SIZE = 64
EPOCHS = 100
LSTM_HIDDEN = 128
DROPOUT_RATE = 0.5
NUM_INPUT_CHANNELS = len(BEST_INPUT_CHANNELS)  # 3 input channels
...
def plot_and_save_predictions(y_true, y_pred, save_dir, max_plots=20):
    """Plots Actual vs Predicted EMG signals with bold, large fonts."""
    os.makedirs(save_dir, exist_ok=True)

    # Global font and layout configuration
    plt.rcParams.update({
        'font.size': 14,
        'font.weight': 'bold',
        'axes.labelsize': 18,
...
if __name__ == "__main__":
    ...
    # Create save path for this 3-input combination
    combination_subfolder = (
        f"plots_{BEST_INPUT_CHANNELS[0]}_{BEST_INPUT_CHANNELS[1]}_{BEST_INPUT_CHANNELS[2]}_to_{BEST_TARGET_CHANNEL}"
    )
